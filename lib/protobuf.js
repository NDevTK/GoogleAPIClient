 ...
 * Uses Number (safe to 2^53-1 — sufficient for field numbers, lengths, codes).
 */
...
function pbTreeNode(f, depth) {
  var node = { field: f.field, wire: f.wire };

  if (f.wire === PB_VARINT) {
    node.value = f.data;
    // ZigZag decode for signed interpretation
    node.asSigned = (f.data >>> 1) ^ -(f.data & 1);
    return node;
  }
  ...
  if (f.wire === PB_LEN) {
    ...
    if (depth > 0 && f.data.length > 0) {
      try {
        var nested = pbDecodeRaw(f.data);
        if (nested.length > 0) {
          // Sanity: all field numbers should be reasonable
          var valid = true;
