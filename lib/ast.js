...
// Extract the source line containing a node for display in the UI.
// Returns the trimmed line text (capped at 120 chars).
function _extractCodeContext(node) {
  if (!_sourceCode || !node || node.start == null) return null;
  // Find the line containing node.start
  var lineStart = _sourceCode.lastIndexOf("\n", node.start - 1) + 1;
  var lineEnd = _sourceCode.indexOf("\n", node.start);
  if (lineEnd === -1) lineEnd = _sourceCode.length;
  var line = _sourceCode.substring(lineStart, lineEnd).trim();
...
function _traceValueSource(path, depth) {
  ...
  if (_t.isMemberExpression(node) && node.computed) {
    var compObjSource = _traceValueSource(path.get("object"), depth);
...
function _processSecurityAssignSink(path, result) {
  ...
  result.securitySinks.push({
    ...
    severity: "high",
    codeContext: _extractCodeContext(node),
  });
}
