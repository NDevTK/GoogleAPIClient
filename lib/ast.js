...
function _resolveCalleeToFunction(callPath) {
  ...
  if (commonPath) return commonPath.node;
...
function _traceReturnsInIfChain(ifPath) {
  if (!ifPath || !ifPath.node || !_t.isIfStatement(ifPath.node)) return null;
  var stmt = ifPath.node;
  if (stmt.consequent && _t.isBlockStatement(stmt.consequent)) {
    var _r = _traceReturnsInBlock(ifPath.get("consequent"));
    if (_r && _r.sourceType === "user-controlled") return _r;
  }
  if (stmt.alternate) {
    if (_t.isBlockStatement(stmt.alternate)) {
      var _r2 = _traceReturnsInBlock(ifPath.get("alternate"));
      if (_r2 && _r2.sourceType === "user-controlled") return _r2;
    } else if (_t.isIfStatement(stmt.alternate)) {
      return _traceReturnsInIfChain(ifPath.get("alternate"));
    }
  }
  return null;
}
    ...
    if (objPropName) {
      ...
      if (objBinding && objBinding.path.isVariableDeclarator() && objBinding.path.node.init) {
        ...
        if (_t.isCallExpression(_objInit) && _t.isMemberExpression(_objInit.callee) &&
            ...
            _t.isIdentifier(_objInit.callee.object, { name: "Object" }) && _objInit.arguments.length >= 2) {
        ...
        if (paramIdx >= 0 && funcPath) {
          ...
          if (paramIdx === 0) {
            ...
            if (_iterParent && _iterParent.isCallExpression() &&
              ...
              if (_iterMethod === "forEach" || _iterMethod === "map" || _iterMethod === "filter" ||
                  _iterMethod === "some" || _iterMethod === "every" || _iterMethod === "find" ||
                  _iterMethod === "findIndex" || _iterMethod === "flatMap" ||
                  _iterMethod === "then" || _iterMethod === "catch") {
            ...
            if (_iterBinding && _iterBinding.referencePaths) {
              for (var _iri = 0; _iri < _iterBinding.referencePaths.length; _iri++) {
                ...
                if (_irParent && _irParent.isCallExpression() &&
                  ...
                  if (_irMethod === "forEach" || _irMethod === "map" || _irMethod === "filter" ||
                      _irMethod === "some" || _irMethod === "every" || _irMethod === "find" ||
                      _irMethod === "findIndex" || _irMethod === "flatMap" ||
                      _irMethod === "then" || _irMethod === "catch") {
...
function _pushSink(result, node, type, sink, src) {
  result.securitySinks.push({
    type: type, sink: sink, location: _nodeLoc(node),
    sourceType: src.sourceType, source: src.source, severity: "high",
...
function _processSecurityAssignSink(path, result) {
  ...
  if (_t.isIdentifier(left, { name: "location" }) && !path.scope.getBinding("location")) {
    ...
    _pushSink(result, node, "redirect", "location", _bLocSrc);
    return;
  }
  ...
  _pushSink(result, node, sinkType, propName, valueSource);
}
...
function _processSecurityCallSink(path, result) {
  ...
  if (callee.type === "Import" && node.arguments.length > 0) {
    ...
    if (_impSrc.sourceType === "user-controlled") _pushSink(result, node, "eval", "import", _impSrc);
    return;
  }
  ...
  if (_t.isSequenceExpression(callee) && callee.expressions.length > 0 && node.arguments.length > 0) {
    ...
    if (_t.isIdentifier(_seqLast, { name: "eval" }) && !path.scope.getBinding("eval")) {
      ...
      if (_indEvalSrc.sourceType === "user-controlled") _pushSink(result, node, "eval", "eval", _indEvalSrc);
      return;
    }
  }
  ...
  if (_t.isIdentifier(callee) && node.arguments.length > 0) {
    ...
    if (_gSink && !path.scope.getBinding(callee.name)) {
      ...
      if (_gSrc.sourceType === "user-controlled") _pushSink(result, node, _gSink.type, _gSink.sink, _gSrc);
      return;
    }
  }
  ...
  if (_t.isIdentifier(callee) && (callee.name === "setTimeout" || callee.name === "setInterval") &&
    ...
    if (!isFunc) {
      ...
      _pushSink(result, node, "eval", callee.name, timerSource);
      return;
    }
  }
  ...
  if (methName === "open" && node.arguments.length > 0) {
    ...
    if (_isWin) {
      ...
      if (_woSrc2.sourceType === "user-controlled") _pushSink(result, node, "redirect", "window.open", _woSrc2);
      return;
    }
  }
  ...
  if (methName === "setHTMLUnsafe" || methName === "parseHTMLUnsafe" || methName === "createContextualFragment") {
    if (node.arguments.length > 0) {
      ...
      if (_htmlSrc.sourceType === "user-controlled") _pushSink(result, node, "xss", methName, _htmlSrc);
    }
    return;
  }
  ...
  if ((methName === "write" || methName === "writeln") && _t.isIdentifier(callee.object, { name: "document" }) &&
    ...
    if (dwSource.sourceType === "user-controlled") _pushSink(result, node, "xss", "document." + methName, dwSource);
    return;
  }
  ...
  if (methName === "insertAdjacentHTML" && node.arguments.length >= 2) {
    ...
    if (iahSource.sourceType === "user-controlled") _pushSink(result, node, "xss", "insertAdjacentHTML", iahSource);
    return;
  }
  ...
  if (methName === "setAttribute" && node.arguments.length >= 2 && _t.isStringLiteral(node.arguments[0])) {
    ...
    if (attrName.startsWith("on") || attrName === "href" || attrName === "src" ||
      ...
      if (saSource.sourceType === "user-controlled") _pushSink(result, node, "xss", "setAttribute:" + attrName, saSource);
      return;
    }
  }
  ...
  if ((methName === "html" || methName === "append" || methName === "prepend" ||
    ...
    if (_jqSrc.sourceType === "user-controlled") _pushSink(result, node, "xss", "." + methName, _jqSrc);
    return;
  }
  ...
  if (methName === "sendBeacon" && node.arguments.length > 0 &&
    ...
    if (_sbSrc.sourceType === "user-controlled") _pushSink(result, node, "request-forgery", "navigator.sendBeacon", _sbSrc);
    return;
  }
  ...
  if (methName === "open" && node.arguments.length >= 2) {
    ...
    if (_isXhrOpen) {
      ...
      if (_xhrUrlSrc.sourceType === "user-controlled") _pushSink(result, node, "request-forgery", "XMLHttpRequest.open", _xhrUrlSrc);
    }
    return;
  }
  ...
  if (methName === "register" && node.arguments.length > 0 &&
    ...
    if (_t.isIdentifier(_swBase, { name: "navigator" }) && !path.scope.getBinding("navigator")) {
      ...
      if (_swSrc.sourceType === "user-controlled") _pushSink(result, node, "eval", "serviceWorker.register", _swSrc);
      return;
    }
  }
  ...
  if ((methName === "assign" || methName === "replace") && node.arguments.length > 0) {
    ...
    _pushSink(result, node, "redirect", "location." + methName, locSource);
  }
}
...
function _processSecurityNewSink(path, result) {
  ...
  if (ctorName === "Function") {
    ...
    if (!_t.isStringLiteral(lastArg)) {
      ...
      if (fnSource.sourceType === "user-controlled") _pushSink(result, node, "eval", "new Function", fnSource);
    }
    return;
  }
...
function _processReactDangerousHTML(path, result) {
  ...
  for (var i = 0; i < node.properties.length; i++) {
    ...
    for (var j = 0; j < prop.value.properties.length; j++) {
      ...
      if (htmlSrc.sourceType === "user-controlled") _pushSink(result, node, "xss", "dangerouslySetInnerHTML", htmlSrc);
    }
  }
}
...
function _processDangerousPattern(path, result) {
  ...
  if (_t.isMemberExpression(callee) && _t.isIdentifier(callee.property, { name: "addEventListener" }) &&
    ...
    var handlerFunc = _resolveHandlerFunc(node.arguments[1], path);
    if (handlerFunc && handlerFunc.body) _classifyAndReportMessageHandler(handlerFunc, node, result);
    return;
  }
...
function _resolveHandlerFunc(handler, path) {
  ...
  if (_t.isCallExpression(handler)) {
    ...
    var _rhFuncNode = null;
    if (_t.isIdentifier(_rhCallee)) {
      var _rhBind = path.scope.getBinding(_rhCallee.name);
      if (_rhBind) {
        if (_t.isFunctionDeclaration(_rhBind.path.node)) _rhFuncNode = _rhBind.path.node;
        else if (_rhBind.path.isVariableDeclarator() && _rhBind.path.node.init && _t.isFunction(_rhBind.path.node.init))
          _rhFuncNode = _rhBind.path.node.init;
      }
    }
    if (_rhFuncNode && _rhFuncNode.body && _t.isBlockStatement(_rhFuncNode.body)) {
      // Find the returned function in the body
      var _rhBody = _rhFuncNode.body.body;
      for (var _rhi = 0; _rhi < _rhBody.length; _rhi++) {
        if (_t.isReturnStatement(_rhBody[_rhi]) && _rhBody[_rhi].argument) {
          ...
          if (_t.isFunctionExpression(_rhRet) || _t.isArrowFunctionExpression(_rhRet)) return _rhRet;
        }
      }
    }
  }
...
function _processDangerousAssignment(path, result) {
  ...
  if (_t.isMemberExpression(left) && !left.computed && _t.isIdentifier(left.property, { name: "onmessage" })) {
    ...
    if (_isGlobal) {
      var _omResolved = _resolveHandlerFunc(node.right, path);
      if (_omResolved && _omResolved.body) _classifyAndReportMessageHandler(_omResolved, node, result);
      return;
    }
  }
