...
var _TAINT_SOURCES = {
  ...
  "document.cookie": 1, "document.domain": 1, "document.title": 1,
...
function _processSecurityAssignSink(path, result) {
  ...
  if (_t.isIdentifier(left, { name: "location" }) && !path.scope.getBinding("location")) {
    ...
    result.securitySinks.push({
      type: "redirect", sink: "location",
      location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
      sourceType: _bLocSrc.sourceType, source: _bLocSrc.source, severity: "high",
      codeContext: _extractCodeContext(node, _bLocSrc),
    });
    return;
  }
  ...
  // Determine if the object is a location reference (location, window.location, self.location, document.location)
  var _isLoc = _t.isIdentifier(left.object, { name: "location" }) && !path.scope.getBinding("location");
  if (!_isLoc && _t.isMemberExpression(left.object) && !left.object.computed &&
      _t.isIdentifier(left.object.property, { name: "location" })) {
    var _lb = left.object.object;
    if ((_t.isIdentifier(_lb, { name: "window" }) && !path.scope.getBinding("window")) ||
        (_t.isIdentifier(_lb, { name: "self" }) && !path.scope.getBinding("self")) ||
        (_t.isIdentifier(_lb, { name: "document" }) && !path.scope.getBinding("document"))) {
      _isLoc = true;
    }
  }
...
function _processSecurityCallSink(path, result) {
  ...
  if (callee.type === "Import" && node.arguments.length > 0) {
    ...
    if (_impSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "eval", sink: "import",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _impSrc.sourceType, source: _impSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _impSrc),
      });
    }
    return;
  }
  ...
  // open(url) / window.open(url) / self.open(url) — open redirect
  if (_t.isIdentifier(callee, { name: "open" }) && !path.scope.getBinding("open") && node.arguments.length > 0) {
    var _woSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_woSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "redirect", sink: "window.open",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _woSrc.sourceType, source: _woSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _woSrc),
      });
    }
    return;
  }

  // fetch(url) — client-side request forgery. User-controlled URL enables data exfiltration,
  // internal network probing, credential forwarding (especially with credentials: "include").
  if (_t.isIdentifier(callee, { name: "fetch" }) && !path.scope.getBinding("fetch") && node.arguments.length > 0) {
    var _fetchUrlSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_fetchUrlSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "request-forgery", sink: "fetch",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _fetchUrlSrc.sourceType, source: _fetchUrlSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _fetchUrlSrc),
      });
    }
    return;
  }

  // importScripts(url) — script injection in worker context
  if (_t.isIdentifier(callee, { name: "importScripts" }) && !path.scope.getBinding("importScripts") && node.arguments.length > 0) {
    var _isSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_isSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "eval", sink: "importScripts",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _isSrc.sourceType, source: _isSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _isSrc),
      });
    }
    return;
  }
    ...
    if (_isWin) {
      ...
      if (_woSrc2.sourceType === "user-controlled") {
        result.securitySinks.push({
          type: "redirect", sink: "window.open",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          sourceType: _woSrc2.sourceType, source: _woSrc2.source, severity: "high",
          codeContext: _extractCodeContext(node, _woSrc2),
        });
      }
      return;
    }
  ...
  // element.setHTMLUnsafe(html) — DOM XSS via unsanitized HTML parsing
  if (methName === "setHTMLUnsafe" && node.arguments.length > 0) {
    var _shSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_shSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "xss", sink: "setHTMLUnsafe",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _shSrc.sourceType, source: _shSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _shSrc),
      });
    }
    return;
  }

  // Document.parseHTMLUnsafe(html) — XSS via unsanitized HTML document creation
  if (methName === "parseHTMLUnsafe" && node.arguments.length > 0) {
    var _phSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_phSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "xss", sink: "parseHTMLUnsafe",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _phSrc.sourceType, source: _phSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _phSrc),
      });
    }
    return;
  }
  ...
  // setAttribute("style", value) — CSS injection
  if (methName === "setAttribute" && node.arguments.length >= 2 && _t.isStringLiteral(node.arguments[0]) &&
      node.arguments[0].value.toLowerCase() === "style") {
    var _cssSrc = _traceValueSource(path.get("arguments.1"), 0);
    if (_cssSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "xss", sink: "setAttribute:style",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _cssSrc.sourceType, source: _cssSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _cssSrc),
      });
    }
    return;
  }
  ...
  // These parse HTML strings and insert into the DOM — equivalent to innerHTML when called with strings.
  // Method names survive minification (property access on unknown objects cannot be renamed).
  if ((methName === "html" || methName === "append" || methName === "prepend" ||
    ...
    if (_jqSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "xss", sink: "." + methName,
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _jqSrc.sourceType, source: _jqSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _jqSrc),
      });
    }
    return;
  }

  // Implicit ReDoS: .match(), .search(), .split() with user-controlled first arg.
  // These implicitly construct RegExp from string arguments, enabling ReDoS.
  if ((methName === "match" || methName === "search" || methName === "split") &&
    ...
    if (_reImplSrc.sourceType === "user-controlled") {
      result.dangerousPatterns.push({
        type: "regex-implicit",
        description: "String." + methName + " with user-controlled pattern (implicit RegExp, potential ReDoS)",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        severity: "medium",
        codeContext: _extractCodeContext(node, _reImplSrc),
      });
    }
    return;
  }
  ...
  // Survives page unload, sends cookies, most reliable exfiltration channel.
  if (methName === "sendBeacon" && node.arguments.length > 0 &&
    ...
    if (_sbSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "request-forgery", sink: "navigator.sendBeacon",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _sbSrc.sourceType, source: _sbSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _sbSrc),
      });
    }
    return;
  }
  ...
  if (methName === "open" && node.arguments.length >= 2) {
    // Heuristic: .open() with 2+ args where second arg is tainted — XHR pattern
    ...
    if (_isXhrOpen) {
      ...
      if (_xhrUrlSrc.sourceType === "user-controlled") {
        result.securitySinks.push({
          type: "request-forgery", sink: "XMLHttpRequest.open",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          sourceType: _xhrUrlSrc.sourceType, source: _xhrUrlSrc.source, severity: "high",
          codeContext: _extractCodeContext(node, _xhrUrlSrc),
        });
      }
    }
    return;
  ...
  if (methName === "register" && node.arguments.length > 0 &&
    ...
    if (_t.isIdentifier(_swBase, { name: "navigator" }) && !path.scope.getBinding("navigator")) {
      ...
      if (_swSrc.sourceType === "user-controlled") {
        result.securitySinks.push({
          type: "eval", sink: "serviceWorker.register",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          sourceType: _swSrc.sourceType, source: _swSrc.source, severity: "high",
          codeContext: _extractCodeContext(node, _swSrc),
        });
      }
      return;
    }
  ...
  // Also handles window.location.assign() / self.location.assign() / document.location.assign()
  if ((methName === "assign" || methName === "replace") && node.arguments.length > 0) {
    ...
    if (!isLocObj && _t.isMemberExpression(callee.object) && !callee.object.computed &&
      ...
      if ((_t.isIdentifier(lBase, { name: "window" }) && !path.scope.getBinding("window")) ||
          ...
          (_t.isIdentifier(lBase, { name: "document" }) && !path.scope.getBinding("document"))) {
        isLocObj = true;
      }
    }
    if (!isLocObj) return;
...
function _processSecurityNewSink(path, result) {
  ...
  // new Worker(url) / new SharedWorker(url) — worker injection
  if ((_t.isIdentifier(node.callee, { name: "Worker" }) && !path.scope.getBinding("Worker")) ||
      (_t.isIdentifier(node.callee, { name: "SharedWorker" }) && !path.scope.getBinding("SharedWorker"))) {
    if (node.arguments.length > 0 && !_t.isStringLiteral(node.arguments[0])) {
      var _wkSrc = _traceValueSource(path.get("arguments.0"), 0);
      if (_wkSrc.sourceType === "user-controlled") {
        result.securitySinks.push({
          type: "eval", sink: "new " + node.callee.name,
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          sourceType: _wkSrc.sourceType, source: _wkSrc.source, severity: "high",
          codeContext: _extractCodeContext(node, _wkSrc),
        });
      }
    }
  }

  // new WebSocket(url) — no CORS protection, persistent bidirectional connection.
  // User-controlled URL enables connection hijacking, data exfiltration, malicious data injection.
  if (_t.isIdentifier(node.callee, { name: "WebSocket" }) && !path.scope.getBinding("WebSocket") &&
      node.arguments.length > 0 && !_t.isStringLiteral(node.arguments[0])) {
    var _wsSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_wsSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "request-forgery", sink: "new WebSocket",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _wsSrc.sourceType, source: _wsSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _wsSrc),
      });
    }
  }

  // new EventSource(url) — no CORS preflight, persistent connection, sends cookies.
  // User-controlled URL enables credential forwarding and malicious data injection.
  if (_t.isIdentifier(node.callee, { name: "EventSource" }) && !path.scope.getBinding("EventSource") &&
      node.arguments.length > 0 && !_t.isStringLiteral(node.arguments[0])) {
    var _esSrc = _traceValueSource(path.get("arguments.0"), 0);
    if (_esSrc.sourceType === "user-controlled") {
      result.securitySinks.push({
        type: "request-forgery", sink: "new EventSource",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        sourceType: _esSrc.sourceType, source: _esSrc.source, severity: "high",
        codeContext: _extractCodeContext(node, _esSrc),
      });
    }
  }
}
...
function _processReactDangerousHTML(path, result) {
  ...
  for (var i = 0; i < node.properties.length; i++) {
    var prop = node.properties[i];
    if (!_t.isObjectProperty(prop) || prop.computed) continue;
    var keyName = _t.isIdentifier(prop.key) ? prop.key.name : (_t.isStringLiteral(prop.key) ? prop.key.value : null);
    if (keyName !== "dangerouslySetInnerHTML") continue;
    // Found dangerouslySetInnerHTML — check for __html in the nested object
    ...
    for (var j = 0; j < prop.value.properties.length; j++) {
      ...
      var innerKey = _t.isIdentifier(inner.key) ? inner.key.name : (_t.isStringLiteral(inner.key) ? inner.key.value : null);
      if (innerKey !== "__html") continue;
      var htmlSrc = _traceValueSource(path.get("properties." + i + ".value.properties." + j + ".value"), 0);
      if (htmlSrc.sourceType === "user-controlled") {
        result.securitySinks.push({
          type: "xss", sink: "dangerouslySetInnerHTML",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          sourceType: htmlSrc.sourceType, source: htmlSrc.source, severity: "high",
          codeContext: _extractCodeContext(node, htmlSrc),
        });
      }
    }
  }
...
function _processDangerousPattern(path, result) {
  ...
  if (_t.isMemberExpression(callee) && _t.isIdentifier(callee.property, { name: "addEventListener" }) &&
    ...
    var handlerFunc = _resolveHandlerFunc(handler, path);
    if (handlerFunc && handlerFunc.body) {
      _classifyAndReportMessageHandler(handlerFunc, node, result);
    }
    return;
  }
  ...
  if (_t.isMemberExpression(callee) && _t.isIdentifier(callee.property, { name: "postMessage" }) &&
    ...
    result.dangerousPatterns.push({
      type: "postmessage-wildcard-target",
      description: _isOpener ? "postMessage to opener with wildcard '*' targetOrigin" : "postMessage with wildcard '*' targetOrigin",
      location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
      severity: "high",
      codeContext: _extractCodeContext(node, null),
    });
    return;
  }
  ...
  if (_t.isMemberExpression(callee) && !callee.computed &&
    ...
    if (_dpKeySrc.sourceType === "user-controlled") {
      result.dangerousPatterns.push({
        ...
        description: "Object.defineProperty with user-controlled key",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        severity: "high",
        codeContext: _extractCodeContext(node, _dpKeySrc),
      });
    }
    return;
  }
  ...
  if (_t.isMemberExpression(callee) && !callee.computed &&
    ...
    if (_rsKeySrc.sourceType === "user-controlled") {
      result.dangerousPatterns.push({
        type: "prototype-pollution",
        description: "Reflect.set with user-controlled key",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        severity: "high",
        codeContext: _extractCodeContext(node, _rsKeySrc),
      });
    }
    return;
  }
  ...
  if (_t.isMemberExpression(callee) && !callee.computed &&
      ...
      _t.isIdentifier(callee.object, { name: "Object" }) && !path.scope.getBinding("Object") &&
      node.arguments.length >= 2) {
    // Check all source arguments (skip the first which is the target)
    for (var _oaI = 1; _oaI < node.arguments.length; _oaI++) {
      ...
      if (_oaSrc.sourceType === "user-controlled") {
        result.dangerousPatterns.push({
          type: "prototype-pollution-merge",
          description: "Object.assign with user-controlled source object",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          severity: "medium",
          codeContext: _extractCodeContext(node, _oaSrc),
        });
        break;
      }
    }
  ...
  if (_t.isMemberExpression(callee) && !callee.computed &&
    ...
    for (var _tti = 0; _tti < _ttProps.length; _tti++) {
      ...
      var _ttKey = _t.isIdentifier(_ttProp.key) ? _ttProp.key.name : (_t.isStringLiteral(_ttProp.key) ? _ttProp.key.value : null);
      ...
      if (_t.isArrowFunctionExpression(_ttFn) && _ttFn.params.length === 1 && _t.isIdentifier(_ttFn.params[0])) {
        var _paramName = _ttFn.params[0].name;
        // Arrow with expression body: (s) => s
        if (_t.isIdentifier(_ttFn.body, { name: _paramName })) _isIdentityFn = true;
        // Arrow with block body: (s) => { return s; }
        if (_t.isBlockStatement(_ttFn.body) && _ttFn.body.body.length === 1 &&
            _t.isReturnStatement(_ttFn.body.body[0]) &&
            _t.isIdentifier(_ttFn.body.body[0].argument, { name: _paramName })) _isIdentityFn = true;
      }
      if (_t.isFunctionExpression(_ttFn) && _ttFn.params.length === 1 && _t.isIdentifier(_ttFn.params[0])) {
        var _fParamName = _ttFn.params[0].name;
        if (_t.isBlockStatement(_ttFn.body) && _ttFn.body.body.length === 1 &&
            _t.isReturnStatement(_ttFn.body.body[0]) &&
            _t.isIdentifier(_ttFn.body.body[0].argument, { name: _fParamName })) _isIdentityFn = true;
      }
      if (_isIdentityFn) {
        result.dangerousPatterns.push({
          type: "trusted-types-passthrough",
          description: "Trusted Types policy with passthrough " + _ttKey + " (defeats Trusted Types protection)",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          severity: "high",
          codeContext: _extractCodeContext(node, null),
        });
        break; // One finding per createPolicy call is enough
      }
    }
    return;
  }
...
function _resolveHandlerFunc(handler, path) {
  if (_t.isFunctionExpression(handler) || _t.isArrowFunctionExpression(handler)) return handler;
  if (_t.isIdentifier(handler)) {
    var hBinding = path.scope.getBinding(handler.name);
    if (hBinding) {
      if (_t.isFunctionDeclaration(hBinding.path.node)) return hBinding.path.node;
      if (hBinding.path.node.init && (_t.isFunctionExpression(hBinding.path.node.init) || _t.isArrowFunctionExpression(hBinding.path.node.init)))
        return hBinding.path.node.init;
    }
  }
  return null;
}
...
function _classifyAndReportMessageHandler(handlerFunc, eventNode, result) {
  var body = _t.isBlockStatement(handlerFunc.body) ? handlerFunc.body : handlerFunc.body;
  var classification = _classifyOriginCheck(body);
  var loc = { line: eventNode.loc ? eventNode.loc.start.line : 0, column: eventNode.loc ? eventNode.loc.start.column : 0 };
  var ctx = _extractCodeContext(eventNode, { sourceLoc: handlerFunc.loc ? { line: handlerFunc.body.loc ? handlerFunc.body.loc.end.line : handlerFunc.loc.end.line } : null });

  if (classification === "none") {
    result.dangerousPatterns.push({
      type: "postmessage-no-origin",
      description: "postMessage listener without origin check",
      location: loc, severity: "high", codeContext: ctx,
    });
  } else if (classification === "source-only") {
    result.dangerousPatterns.push({
      type: "postmessage-no-origin",
    ...
    result.dangerousPatterns.push({
      type: "postmessage-weak-origin",
      description: "postMessage listener with bypassable origin check (use strict === comparison)",
      location: loc, severity: "high", codeContext: ctx,
    });
  }
  // "strong" → no finding
...
function _classifyOriginCheck(bodyNode) {
  if (!bodyNode) return "none";
  var hasStrongOrigin = false;
  var hasWeakOrigin = false;
  var hasSourceOnly = false;

  function walk(node) {
    if (hasStrongOrigin || !node || typeof node !== "object") return;
    // Strong: binary === / !== / == / != with .origin
    if (node.type === "BinaryExpression" &&
        (node.operator === "===" || node.operator === "!==" || node.operator === "==" || node.operator === "!=")) {
      if (_hasPropertyMember(node.left, "origin") || _hasPropertyMember(node.right, "origin")) {
        hasStrongOrigin = true; return;
      }
      if (_hasPropertyMember(node.left, "source") || _hasPropertyMember(node.right, "source")) {
        hasSourceOnly = true;
      }
    }
    // Weak: method call on .origin (indexOf, includes, startsWith, endsWith)
    if (node.type === "CallExpression" && node.callee && node.callee.type === "MemberExpression") {
      var mn = node.callee.property && node.callee.property.name;
      if (mn && (mn === "indexOf" || mn === "includes" || mn === "startsWith" || mn === "endsWith")) {
        if (_hasPropertyMember(node.callee.object, "origin")) {
          hasWeakOrigin = true;
        }
      }
    }
    // Walk children — must cover all expression/statement types to find nested patterns
    if (node.type === "IfStatement") { if (node.test) walk(node.test); if (node.consequent) walk(node.consequent); if (node.alternate) walk(node.alternate); }
    if (node.type === "LogicalExpression" || node.type === "BinaryExpression") { walk(node.left); walk(node.right); }
    if (node.type === "UnaryExpression" && node.argument) walk(node.argument);
    if (node.type === "CallExpression") { if (node.callee) walk(node.callee); }
    if (node.type === "MemberExpression") { walk(node.object); }
    ...
    if (node.type === "AssignmentExpression") { walk(node.left); walk(node.right); }
  }
...
function _processDangerousAssignment(path, result) {
  ...
  if (_t.isMemberExpression(left) && !left.computed && _t.isIdentifier(left.property, { name: "onmessage" })) {
    ...
    if (_isGlobal) {
      var _omFunc = node.right;
      if (_t.isFunctionExpression(_omFunc) || _t.isArrowFunctionExpression(_omFunc)) {
        if (_omFunc.body) {
          _classifyAndReportMessageHandler(_omFunc, node, result);
        }
      } else if (_t.isIdentifier(_omFunc)) {
        var _omBind = path.scope.getBinding(_omFunc.name);
        if (_omBind) {
          var _omResolved = null;
          if (_t.isFunctionDeclaration(_omBind.path.node)) _omResolved = _omBind.path.node;
          else if (_omBind.path.node.init && (_t.isFunctionExpression(_omBind.path.node.init) || _t.isArrowFunctionExpression(_omBind.path.node.init)))
            _omResolved = _omBind.path.node.init;
          if (_omResolved && _omResolved.body) {
            _classifyAndReportMessageHandler(_omResolved, node, result);
          }
        }
      }
      return;
