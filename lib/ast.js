...
function _extractBodyParams(valNode, scopePath, _depth) {
  if (!_depth) _depth = 0;
  var params = [];
  // Resolve identifiers through scope (variable reference or function parameter)
  if (_t.isIdentifier(valNode) && scopePath && scopePath.scope && _depth < 3) {
    ...
    if (bpBinding) {
      if (_t.isVariableDeclarator(bpBinding.path.node) && bpBinding.path.node.init) {
        return _extractBodyParams(bpBinding.path.node.init, bpBinding.path, _depth + 1);
      }
          ...
          if (bpIdx >= 0) {
            for (var bri = 0; bri < bpFuncB.referencePaths.length; bri++) {
              ...
              if (_t.isCallExpression(bRef.parent) && bRef.parent.callee === bRef.node &&
                ...
                var bCallerArg = _extractBodyParams(bRef.parent.arguments[bpIdx], bRef.parentPath, _depth + 1);
  ...
  if (_isJsonStringify(valNode, scopePath)) {
    ...
    } else if (valNode.arguments[0] && _t.isIdentifier(valNode.arguments[0]) && scopePath && _depth < 3) {
      // JSON.stringify(identifier) — resolve the identifier
      params = _extractBodyParams(valNode.arguments[0], scopePath, _depth + 1);
    }
...
function _traceValueSource(path, depth) {
  if (!path || !path.node) return { sourceType: "dynamic", source: null };
  if (depth > 3) return { sourceType: "dynamic", source: null };
  var node = path.node;
  ...
  if (_t.isMemberExpression(node) && !node.computed) {
    ...
    if (desc && _TAINT_SOURCES[desc]) {
      return { sourceType: "user-controlled", source: desc };
    }
    ...
    if (_t.isIdentifier(node.object, { name: "location" }) && !path.scope.getBinding("location")) {
      return { sourceType: "user-controlled", source: "location." + (_t.isIdentifier(node.property) ? node.property.name : "*") };
    }
  }
  ...
  if (_t.isIdentifier(node)) {
    ...
    if (binding) {
      ...
      if (binding.path.isVariableDeclarator() && binding.path.node.init) {
        return _traceValueSource(binding.path.get("init"), depth + 1);
      }
      // Function parameter — check callers for user-controlled values
      if (binding.kind === "param" && depth < 2) {
        var paramIdx = -1;
        var funcPath = binding.scope.path;
        ...
        if (paramIdx >= 0 && funcPath) {
          ...
          for (var ci = 0; ci < callerArgs.length; ci++) {
            if (paramIdx < callerArgs[ci].length) {
              var argSource = _traceValueSource(callerArgs[ci][paramIdx], depth + 1);
  ...
  if (_t.isTemplateLiteral(node)) {
    for (var ti = 0; ti < node.expressions.length; ti++) {
      var exprSource = _traceValueSource(path.get("expressions." + ti), depth);
  ...
  if (_t.isBinaryExpression(node) && node.operator === "+") {
    var leftSource = _traceValueSource(path.get("left"), depth);
    if (leftSource.sourceType === "user-controlled") return leftSource;
    var rightSource = _traceValueSource(path.get("right"), depth);
  ...
  if (_t.isConditionalExpression(node)) {
    var consSource = _traceValueSource(path.get("consequent"), depth);
    if (consSource.sourceType === "user-controlled") return consSource;
    var altSource = _traceValueSource(path.get("alternate"), depth);
  ...
  if (_t.isCallExpression(node)) {
    ...
    if (_t.isMemberExpression(node.callee) && !node.callee.computed) {
      var callObjSource = _traceValueSource(path.get("callee.object"), depth);
    ...
    if (node.arguments.length > 0) {
      var argSource = _traceValueSource(path.get("arguments.0"), depth);
      if (argSource.sourceType === "user-controlled") return argSource;
    }
    // new URL(x).searchParams, URLSearchParams(location.search)
    if (_t.isIdentifier(node.callee, { name: "URLSearchParams" }) || _t.isIdentifier(node.callee, { name: "decodeURIComponent" }) ||
        _t.isIdentifier(node.callee, { name: "decodeURI" }) || _t.isIdentifier(node.callee, { name: "atob" }) ||
        _t.isIdentifier(node.callee, { name: "unescape" })) {
      if (node.arguments.length > 0) {
        var innerSource = _traceValueSource(path.get("arguments.0"), depth + 1);
        if (innerSource.sourceType === "user-controlled") return innerSource;
      }
    }
  }
  ...
  if (_t.isAssignmentExpression(node)) {
    return _traceValueSource(path.get("right"), depth);
  }
...
// Assignment-based sinks: element.innerHTML = value, element.outerHTML = value
function _processSecurityAssignSink(path, result) {
  ...
  // Open redirect sinks: location.href = value
  if (!sinkType && _t.isIdentifier(left.object, { name: "location" }) && !path.scope.getBinding("location")) {
    if (propName === "href" || propName === "pathname" || propName === "search" || propName === "hash") {
      sinkType = "redirect";
    }
  }
  if (!sinkType) return;
  ...
  var severity = valueSource.sourceType === "user-controlled" ? "high" :
    (valueSource.sourceType === "dynamic" ? "medium" : "low");
...
function _processSecurityCallSink(path, result) {
  ...
  // eval(value) — must not be locally bound
  if (_t.isIdentifier(callee, { name: "eval" }) && !path.scope.getBinding("eval") && node.arguments.length > 0) {
    ...
    var evalSeverity = evalSource.sourceType === "user-controlled" ? "high" :
      (_t.isStringLiteral(node.arguments[0]) ? "low" : "medium");
    result.securitySinks.push({
      type: "eval", sink: "eval",
      location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
      sourceType: evalSource.sourceType, source: evalSource.source, severity: evalSeverity,
    });
    return;
  }
  ...
  if (_t.isIdentifier(callee) && (callee.name === "setTimeout" || callee.name === "setInterval") &&
    ...
    if (!isFunc) {
      ...
      result.securitySinks.push({
        ...
        severity: timerSource.sourceType === "user-controlled" ? "high" : "medium",
      });
      return;
    }
  }
  ...
  if ((methName === "write" || methName === "writeln") && _t.isIdentifier(callee.object, { name: "document" }) &&
    ...
    result.securitySinks.push({
      ...
      severity: dwSource.sourceType === "user-controlled" ? "high" : (dwSource.sourceType === "dynamic" ? "medium" : "low"),
    });
    return;
  }
  ...
  if (methName === "insertAdjacentHTML" && node.arguments.length >= 2) {
    ...
    result.securitySinks.push({
      ...
      severity: iahSource.sourceType === "user-controlled" ? "high" : (iahSource.sourceType === "dynamic" ? "medium" : "low"),
    });
    return;
  }
  ...
  if (methName === "setAttribute" && node.arguments.length >= 2 && _t.isStringLiteral(node.arguments[0])) {
    ...
    if (attrName.startsWith("on")) {
      ...
      result.securitySinks.push({
        ...
        severity: saSource.sourceType === "user-controlled" ? "high" : "medium",
      });
      return;
    }
  }
  ...
  if ((methName === "assign" || methName === "replace") && _t.isIdentifier(callee.object, { name: "location" }) &&
      !path.scope.getBinding("location") && node.arguments.length > 0) {
    ...
    result.securitySinks.push({
      ...
      severity: locSource.sourceType === "user-controlled" ? "high" : (locSource.sourceType === "dynamic" ? "medium" : "low"),
    });
  }
}
  ...
  if (_t.isIdentifier(node.callee, { name: "Function" }) && !path.scope.getBinding("Function") && node.arguments.length > 0) {
    ...
    if (!_t.isStringLiteral(lastArg)) {
      ...
      result.securitySinks.push({
        ...
        severity: fnSource.sourceType === "user-controlled" ? "high" : "medium",
      });
    }
  }
  ...
  if (_t.isIdentifier(node.callee, { name: "RegExp" }) && !path.scope.getBinding("RegExp") && node.arguments.length > 0) {
    if (!_t.isStringLiteral(node.arguments[0])) {
      ...
      result.dangerousPatterns.push({
        type: "regex-dynamic",
        description: "RegExp constructor with dynamic pattern (potential ReDoS)",
        location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
        severity: reSource.sourceType === "user-controlled" ? "high" : "low",
      });
    }
  }
}
...
function _processDangerousAssignment(path, result) {
  ...
  if (_t.isMemberExpression(left) && left.computed) {
    ...
    if (!_t.isStringLiteral(keyNode) && !_t.isNumericLiteral(keyNode)) {
      ...
      if (keySource.sourceType === "user-controlled" || keySource.sourceType === "dynamic") {
        // Additional check: skip array index patterns (numeric identifiers like i, j, idx)
        if (_t.isIdentifier(keyNode) && /^[ijk]$|idx|index|len/.test(keyNode.name)) return;
        result.dangerousPatterns.push({
          type: "prototype-pollution",
          description: "Dynamic property assignment with " + keySource.sourceType + " key",
          location: { line: node.loc ? node.loc.start.line : 0, column: node.loc ? node.loc.start.column : 0 },
          severity: keySource.sourceType === "user-controlled" ? "high" : "medium",
        });
      }
    }
  }
