...
function buildDiscoveryUrls(hostname, apiKey) {
  ...
  for (const host of hosts) {
    ...
    // 1. Plain GET (public APIs)
    candidates.push({ url: `${base}?_probe=1`, headers: {}, method: "GET" });

    // 2. With visibility label expansion
    candidates.push({
      url: `${base}?labels=PANTHEON&_probe=1`,
      headers: {},
      method: "GET",
    });

    // 3. With explicit version parameter (some services require this)
    for (const ver of versions) {
      candidates.push({
        url: `${base}?version=${ver}&_probe=1`,
        headers: {},
        method: "GET",
      });
    }
    ...
    if (apiKey) {
      ...
      candidates.push({
        url: `${base}?key=${apiKey}&labels=PANTHEON&_probe=1`,
        headers: {},
        method: "GET",
      });
      // API key + version
      for (const ver of versions) {
        candidates.push({
          url: `${base}?key=${apiKey}&version=${ver}&_probe=1`,
          headers: {},
          method: "GET",
        });
      }
    ...
    // 5. POST with X-Http-Method-Override: GET (bypasses 405, "Disclosing YouTube Creator Emails" article)
    candidates.push({
      url: `${base}?_probe=1`,
    ...
    if (apiKey) {
      candidates.push({
        url: `${base}?key=${apiKey}&_probe=1`,
        headers: { "X-Http-Method-Override": "GET", "X-Goog-Api-Key": apiKey },
        method: "POST",
      });
    }
  }

  // 5. Staging sandbox variant
  const baseHost = hostname.replace(".clients6.google.com", ".googleapis.com");
  if (!baseHost.includes("sandbox")) {
    const stagingHost = baseHost
      .replace(".googleapis.com", ".sandbox.googleapis.com")
      .replace(/^([^.]+)\./, "staging-$1.");
    const stagingBase = `https://${stagingHost}/$discovery/rest`;
    candidates.push({
      url: `${stagingBase}?_probe=1`,
      headers: {},
      method: "GET",
    });
    if (apiKey) {
      candidates.push({
        url: `${stagingBase}?key=${apiKey}&_probe=1`,
        headers: {},
        method: "GET",
      });
    }
