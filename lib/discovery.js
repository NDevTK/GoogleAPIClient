...
function summarizeDiscovery(doc) {
  ...
  // Walk probed before learned so probed methods take priority in dedup
  if (doc.resources?.probed) walkResources({ probed: doc.resources.probed }, "");
  if (doc.resources?.learned) walkResources({ learned: doc.resources.learned }, "");
  const skipKeys = new Set(["probed", "learned"]);
  const remaining = {};
  for (const [k, v] of Object.entries(doc.resources || {})) {
    if (!skipKeys.has(k)) remaining[k] = v;
  }
  walkResources(remaining, "");

  // Deduplicate methods across resource groups (e.g. learned + probed)
  const seenMethods = new Set();
  for (const rName of Object.keys(resources)) {
    resources[rName] = resources[rName].filter((m) => {
      const key = `${m.httpMethod} ${m.id}`;
      if (seenMethods.has(key)) return false;
      seenMethods.add(key);
      return true;
    });
    if (resources[rName].length === 0) delete resources[rName];
  }
        ...
        for (const [, method] of Object.entries(resource.methods)) {
          ...
          if (method.parameters) {
            for (const [pName, pDef] of Object.entries(method.parameters)) {
              ...
              if (pDef.number) {
                operation.parameters[operation.parameters.length - 1]["x-field-number"] = pDef.number;
              }
            }
          }
