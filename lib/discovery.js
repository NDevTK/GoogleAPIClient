...
function buildDiscoveryUrls(hostname, apiKey) {
  ...
  for (const path of genericPaths) {
    ...
    candidates.push({ url: `https://${hostname}${path}${separator}_probe=1`, headers: {}, method: "GET" });
  }
...
function convertOpenApiToDiscovery(openapi, sourceUrl) {
  ...
  const doc = {
    ...
    name: openapi.info?.title?.toLowerCase().replace(/[^a-z0-9]/g, "_") || "api",
  ...
  for (const [path, pathDef] of Object.entries(openapi.paths || {})) {
    for (const [method, opDef] of Object.entries(pathDef)) {
      if (["get", "post", "put", "delete", "patch"].includes(method.toLowerCase())) {
        const methodName = opDef.operationId || `${method.toLowerCase()}_${path.replace(/[^a-zA-Z0-9]/g, "_")}`;
        ...
        const params = [...(pathDef.parameters || []), ...(opDef.parameters || [])];
        ...
        // Request Body
        const reqBody = opDef.requestBody?.content?.["application/json"]?.schema;
        if (reqBody?.$ref) {
          m.request = { $ref: reqBody.$ref.split("/").pop() };
        }

        // Response Body
        const respBody = opDef.responses?.["200"]?.content?.["application/json"]?.schema;
        if (respBody?.$ref) {
